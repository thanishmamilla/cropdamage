...
class token_encoder(nn.Module):     #Token Encoder Part of MSCA
    ...
    def forward(self, x):       #spatial module + positional embedding
        b, c, h, w = x.shape
        ...
        tokens += self.pos_embedding        #summation of all heads
...
class context_aggregator(nn.Module):        #Context Aggregation part
...
class Classifier(nn.Module):        #MBPH Part with two Conv2D #x# networks
...
class CDNet(nn.Module):
    def __init__(self,  backbone='resnet18', output_stride=16, img_size = 512, img_chan=3, chan_num = 32, n_class =2):
        ...
        self.backbone = build_backbone(backbone, output_stride, BatchNorm, img_chan)
                                                        ...
                                                        #MSCA Part - Conv2D + Context Aggregation
        ...
        self.conv_s4 = nn.Conv2d(chan_num*2, chan_num, kernel_size=3, padding=1)        #DownSampling 
        ...
        self.classifier3 = Classifier(n_class = n_class)    #Classification at different scales
