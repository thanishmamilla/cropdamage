...
#         print("Time1_dir:",Time1_dir)
#         print("Time2_dir:",Time2_dir)
#         print("Label_dir:",Label_dir)

#         print(args.suffix)
#         print(os.listdir(Time1_dir))
#         print(os.path.splitext('00483.png')[1])
#         print(os.path.splitext('00483.png')[1]==args.suffix)
#         datalist = [name for name in os.listdir(Time1_dir) if
#                     os.path.splitext(name)[1] == args.suffix]
...
class TestDatasetFromFolder(Dataset):
    def __init__(self, args, Time1_dir, Time2_dir, Label_dir=None):  # Allow Label_dir to be None
        super(TestDatasetFromFolder, self).__init__()
        ...
        self.image1_filenames = [join(Time1_dir, x) for x in datalist if is_image_file(x)]
        self.image2_filenames = [join(Time2_dir, x) for x in datalist if is_image_file(x)]
        self.image3_filenames = None if Label_dir is None else [join(Label_dir, x) for x in datalist if
                                                               is_image_file(x)]

        self.transform = get_transform(convert=True, normalize=True)  # convert to tensor and normalize to [-1,1]
        self.label_transform = get_transform() if Label_dir else None  # only convert to tensor if labels are available
...
class DA_DatasetFromFolder(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image1, image2 = self.data_augment(image1, image2)
